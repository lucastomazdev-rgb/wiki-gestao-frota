...
export function useRetiradasData({ avisarMudanca } = {}) {
  ...
  const { data: respostaRetiradas, isLoading: isLoadingRetiradas, isFetching: isFetchingRetiradas } = useQuery({
  ...
  const kpisRetiradas = respostaRetiradas?.kpis || null;

  const retiradas = respostaRetiradas?.data || [];
  const totalRegistros = respostaRetiradas?.pagination?.total ?? respostaRetiradas?.kpis?.total ?? 0;
