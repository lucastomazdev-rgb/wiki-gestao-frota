...
import TarefasBoard, { getCategoryMeta } from './tarefas/TarefasBoard';
...
export default function Tarefas() {
  ...
  const { columnsData, totalDemandas, concluidas, minhasDemandas, activeTask, categoryCounts } = useTarefasDerivedData({
    tasks,
    activeId,
    currentUserId,
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <option value="Configuração/Perfil">Configuração / Perfil</option>
                  ...
                  {/* Micro-destaque e contextualização visual interativa imediata */}
                  {newTask.categoria === 'Configuração/Perfil' && (
                    <div className="mt-2.5 p-3 rounded-xl bg-sky-50/90 border border-sky-200 flex items-center gap-3 animate-in fade-in duration-200">
                      <div className="p-2 bg-sky-100 text-sky-700 rounded-lg shrink-0">
                        <Sliders size={14} strokeWidth={2.4} />
                      </div>
                      <div className="min-w-0">
                        <span className="text-[11px] font-black text-sky-900 block tracking-tight">
                          Destaque: Configuração / Perfil
                        </span>
                        <p className="text-[10px] font-medium text-sky-700 leading-tight">
                          O card no Kanban receberá identificador azul de parametrização e perfil técnico.
                        </p>
                      </div>
                    </div>
                  )}

                  {newTask.categoria === 'Manutenção' && (
                    <div className="mt-2.5 p-3 rounded-xl bg-amber-50/90 border border-amber-200 flex items-center gap-3 animate-in fade-in duration-200">
                      <div className="p-2 bg-amber-100 text-amber-700 rounded-lg shrink-0">
                        <Wrench size={14} strokeWidth={2.4} />
                      </div>
                      <div className="min-w-0">
                        <span className="text-[11px] font-black text-amber-900 block tracking-tight">
                          Destaque: Manutenção
                        </span>
                        <p className="text-[10px] font-medium text-amber-800 leading-tight">
                          O card no Kanban receberá identificador âmbar de intervenção física ou reparo em veículo.
                        </p>
                      </div>
                    </div>
                  )}
              ...
              <div className="pr-8">
                <div className="flex flex-wrap items-center gap-2.5 mb-2.5">
                  ...
                  {selectedTask.categoria && (() => {
                    const meta = getCategoryMeta(selectedTask.categoria);
                    const Icon = meta?.icon;
                    return meta ? (
                      <span className={`px-2.5 py-0.5 text-[9px] font-black uppercase rounded-lg tracking-widest border shadow-xs flex items-center gap-1.5 ${meta.badgeClass}`}>
                      ...
                      {canEditSelectedTask ? (
                        <div className="space-y-1.5">
                          <div className="relative">
                            <select
                              ...
                              onChange={(e) => handleUpdateCategory(selectedTask.id, e.target.value)}
                          ...
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-tight">
                            Você pode alterar a categoria da demanda a qualquer momento.
                          </p>
                        </div>
                      ) : (
                        <div className="flex items-center gap-3 bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
                          {selectedTask.categoria ? (() => {
                            ...
                            const Icon = meta?.icon;
                            return (
                              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold border ${meta?.badgeClass || 'bg-slate-100 text-slate-700'}`}>
                                {Icon && <Icon size={12} strokeWidth={2.4} />}
                                <span>{meta?.label || selectedTask.categoria}</span>
                              </span>
                            );
                          })() : (
                            <span className="text-xs text-slate-400 italic">Nenhuma categoria vinculada</span>
                          )}
                        </div>
                      )}
                    </div>
